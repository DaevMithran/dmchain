...
func (s *IntegrationTestSuite) TestRewardBallotWinners() {
	...
	// Prepare reward pool
	givingAmt := sdk.NewCoins(sdk.NewInt64Coin(types.DmDenom, 30000000))
	...
	for i := 1; i <= 3; i++ {
		voteTargets = append(voteTargets, fmt.Sprintf("%s%d", types.DmSymbol, i))
	}
	...
	s.Require().Equal(sdkmath.LegacyNewDecFromInt(givingAmt.AmountOf(types.DmDenom)).Mul(sdkmath.LegacyMustNewDecFromStr(val1ExpectedRewardFactor).QuoInt64(int64(len(claims)))).QuoInt64(votePeriodsPerWindow).TruncateInt(),
		outstandingRewardsVal1.AmountOf(types.DmDenom))
	s.Require().Equal(sdkmath.LegacyNewDecFromInt(givingAmt.AmountOf(types.DmDenom)).Mul(sdkmath.LegacyMustNewDecFromStr(val2ExpectedRewardFactor).QuoInt64(int64(len(claims)))).QuoInt64(votePeriodsPerWindow).TruncateInt(),
		outstandingRewardsVal2.AmountOf(types.DmDenom))
}
...
func (s *IntegrationTestSuite) TestRewardBallotWinnersZeroMissCounters() {
	...
	s.Require().NoError(err)
	outstandingRewardsVal2, _ := outstandingRewardsDecVal2.TruncateDecimal()
	s.Require().Equal(sdkmath.LegacyNewDecFromInt(givingAmt.AmountOf(types.DmDenom)).QuoInt64(votePeriodsPerWindow).QuoInt64(2).TruncateInt(),
		outstandingRewardsVal1.AmountOf(types.DmDenom))
	s.Require().Equal(sdkmath.LegacyNewDecFromInt(givingAmt.AmountOf(types.DmDenom)).QuoInt64(votePeriodsPerWindow).QuoInt64(2).TruncateInt(),
		outstandingRewardsVal2.AmountOf(types.DmDenom))
}
...
func (s *IntegrationTestSuite) TestRewardBallotWinnersZeroVoteTargets() {
	...
	s.Require().NoError(err)
	outstandingRewardsVal2, _ := outstandingRewardsDecVal2.TruncateDecimal()
	s.Require().Equal(sdkmath.LegacyZeroDec().TruncateInt(), outstandingRewardsVal1.AmountOf(types.DmDenom))
	s.Require().Equal(sdkmath.LegacyZeroDec().TruncateInt(), outstandingRewardsVal2.AmountOf(types.DmDenom))
}
