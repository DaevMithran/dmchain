...
import (
	...
	"github.com/DaevMithran/dmchain/util/genmap"
	"github.com/DaevMithran/dmchain/util/reward"
...
func prependOjoIfUnique(voteTargets []string) []string {
	if genmap.Contains(types.DmDenom, voteTargets) {
		return voteTargets
	}
	rewardDenoms := make([]string, len(voteTargets)+1)
	rewardDenoms[0] = types.DmDenom
